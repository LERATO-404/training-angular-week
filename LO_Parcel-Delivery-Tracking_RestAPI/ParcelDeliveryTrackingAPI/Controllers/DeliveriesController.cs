...
namespace ParcelDeliveryTrackingAPI.Controllers
{
    ...
    public class DeliveriesController : ControllerBase
    {
        ...
        // GET: api/Deliveries/Detailed
        [Route("Detailed")]
        [EnableCors("AllowOrigin")]
        [HttpGet]
        public async Task<ActionResult<List<Delivery>>> GetDetailedDeliveries()
        {
            logger.Info("DeliveriesController - GET: api/Deliveries/Detailed");
            try
            {
                ...
                if(rightRole)
                {
                    ...
                    var deliveries = _deliveryRepository.GetDeliveryDetails();
        ...
        public async Task<ActionResult<List<Delivery>>> GetDetailedDeliveriesByStatus(string status)
        {
            logger.Info("DeliveriesController - GET: api/Deliveries/Detailed");
            try
            {
                DeliveryRepository _deliveryRepository = new DeliveryRepository(_parcelContext);
                var deliveries = _deliveryRepository.GetDetailedDeliveriesByStatus(status);
                return Ok(deliveries);
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> DeleteDelivery(int id)
        {
            ...
            {
                return StatusCode(500, new { message = $"An error occurred while deleting a new Delivery. Try again later." });
            }
        }
    }
}
