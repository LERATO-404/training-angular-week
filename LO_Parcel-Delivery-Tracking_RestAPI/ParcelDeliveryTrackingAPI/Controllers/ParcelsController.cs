...
namespace ParcelDeliveryTrackingAPI.Controllers
{
    ...
    public class ParcelsController : ControllerBase
    {
        ...
        public async Task<ActionResult<List<ParcelDto>>> GetAllParcels()
        {
            ...
            {
                return StatusCode(401, new { message = "Not Authorised." });
            }
        }
        ...
        [EnableCors("AllowOrigin")]
        [HttpGet("Detailed")]
        public async Task<ActionResult<List<ParcelDetailedDto>>> GetDetailedParcels()
        {
            logger.Info("ParcelsController - GET:  api/Parcels/Detailed");
            ...
            try
            {
                ...
                if (rightRole)
                {
                    try
                    {
                        ...
                        ParcelRepository _parcelRepository = new ParcelRepository(_parcelContext);
                        var allParcels = _parcelRepository.GetParcelDetails();
                        return Ok(allParcels);
        ...
        [EnableCors("AllowOrigin")]
        [HttpGet("Delivered")]
        public async Task<ActionResult<List<ParcelDto>>> GetAllParcelsDelivered()
        {
        ...
        [EnableCors("AllowOrigin")]
        [HttpGet("InTransit")]
        public async Task<ActionResult<List<ParcelDto>>> GetAllParcelsInTransit()
        {
        ...
        [EnableCors("AllowOrigin")]
        [HttpGet("OnHold")]
        public async Task<ActionResult<IEnumerable<ParcelDto>>> GetAllParcelsOnHold()
        {
        ...
        public async Task<ActionResult<IEnumerable<ParcelDto>>> GetParcelsByStatus(string status)
        {
            logger.Info($"ParcelsController - GET:  api/Parcels/Status/{status}");
            try
            {
        ...
        [EnableCors("AllowOrigin")]
        [HttpGet("Detailed/Status/{status}")]
        public async Task<ActionResult<IEnumerable<ParcelDetailedDto>>> GetDetailedParcelsByStatus(string status)
        {
            logger.Info($"ParcelsController - GET:  api/Parcels/Detailed/Status/{status}");
            try
            {

                ParcelRepository _parcelRepository = new ParcelRepository(_parcelContext);
                var parcels = _parcelRepository.GetDetailedParcelByStatus(status);
                if (parcels == null)
                {
                    return NotFound(new { message = $"Parcels with status {status} were not found." });
                }
        ...
        public async Task<IActionResult> DeleteParcel(int id)
        {
            ...
            {
                return StatusCode(401, new { message = "Not Authorised. Please contact the Administrator." });
            }
        }
    }
}
