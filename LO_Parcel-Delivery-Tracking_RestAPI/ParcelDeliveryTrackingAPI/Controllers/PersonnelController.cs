...
namespace ParcelDeliveryTrackingAPI.Controllers
{
    ...
    public class PersonnelController : ControllerBase
    {
        ...
        public async Task<ActionResult<List<Personnel>>> GetAllPersonnel()
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    return BadRequest(new { message = "Not Authorised. Please contact the Administrator." });
                }
            }
            catch (Exception ex)
            {
                logger.Error("PersonnelController - GET:  api/Personnel - Not Found / invalid personnel.  Exception: " + ex);
                return BadRequest(new { message = "An error occurred while fetching Personnel." });
            }
        }

        
        // GET: api/Personnel/Drivers
        [EnableCors("AllowOrigin")]
        [HttpGet("Drivers")]
        public async Task<ActionResult<List<Personnel>>> GetAllDrivers()
        {
            logger.Info("PersonnelController - GET:  api/Personnel/Drivers");
            try
            {
                ...
                if (rightRole)
                {
                    ...
                    var usersInRole = await _userManager.GetUsersInRoleAsync(driverRole);
                    var driverUsernames = usersInRole.Select(u => u.UserName);



                    PersonnelRepository _personnelRepository = new PersonnelRepository(_parcelContext);
                    var personnel = _personnelRepository.GetDrivers(driverUsernames);
                    if (personnel == null)
                    {
                        logger.Warn($"PersonnelController - GET:  api/Personnel/Drivers Not Found logged in User: {user}");
                        return NotFound();
                    }
                ...
                {
                    logger.Warn($"PersonnelController - GET:  api/Personnel/Drivers logged in User: {user}");
                    return BadRequest(new { message = "Not Authorised. Please contact the Administrator." });
                }
        ...
        public async Task<ActionResult<IEnumerable<Personnel>>> GetPersonnelByFullName([FromQuery] string firstName, [FromQuery] string lastName)
        {
            ...
            {
                return StatusCode(500, new { message = "Not Authorised. Please contact the Administrator." });
            }
        }
        ...
        public async Task<ActionResult<List<Personnel>>> GetPersonnelOffByAvailability(string availability)
        {
            logger.Info($"PersonnelController - GET:  api/Personnel/{availability}");
            ...
            try
            {
                ...
                {
                    PersonnelRepository _personnelRepository = new PersonnelRepository(_parcelContext);
                    var allPersonnel = _personnelRepository.GetAllPersonnelByAvailability(availability);
                    
                    if(allPersonnel  == null)
                    {
                        return NotFound(new { message = $"No personnel found with the specified availability '{availability}'." });
                    }
            ...
            {
                return StatusCode(500,new { message = $"An error occurred while fetching personnel with availability {availability}." });
            }
        }
